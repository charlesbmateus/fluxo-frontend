<template>
  <div :class="containerClasses">
    <input v-if="isAuthenticated" id="main-drawer" class="drawer-toggle" type="checkbox"/>

    <div class="drawer-content flex flex-col">

      <!-- Navbar -->
      <Navbar>
        <template #actions>
          <NavbarGuestActions v-if="!isAuthenticated"/>
          <NavbarUserActions v-else/>
        </template>
      </Navbar>

      <!-- Page Content -->
      <main class="flex-1 p-4 lg:p-6">
        <slot/>
      </main>
    </div>

    <!-- Sidebar -->
    <div v-if="isAuthenticated" class="drawer-side">
      <label class="drawer-overlay" for="main-drawer"></label>
      <aside :class="isDrawerOpen ? 'w-64' : 'w-20'" class="bg-base-200 min-h-full transition-all duration-300">
        <div class="p-4">
          <div class="flex items-center justify-center py-4">
            <div class="text-center">
              <h2 v-if="isDrawerOpen" class="text-2xl font-bold">
                <span class="text-primary">Fluxo</span>
              </h2>
              <h2 v-else class="text-2xl font-bold">
                <span class="text-primary">F</span>
              </h2>
              <p v-if="isDrawerOpen" class="text-xs text-base-content/60">Marketplace</p>
            </div>
          </div>
        </div>
        <ul class="menu p-4 space-y-2">
          <li>
            <NuxtLink :class="isDrawerOpen ? 'gap-3' : 'justify-center tooltip tooltip-right'" :data-tip="isDrawerOpen ? '' : $t('nav.dashboard')"
                      class="flex items-center"
                      to="/">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5"
                   viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" stroke-linecap="round"
                      stroke-linejoin="round"/>
              </svg>
              <span v-if="isDrawerOpen">{{ $t('nav.dashboard') }}</span>
            </NuxtLink>
          </li>
          <li>
            <NuxtLink :class="isDrawerOpen ? 'gap-3' : 'justify-center tooltip tooltip-right'" :data-tip="isDrawerOpen ? '' : $t('nav.marketplace')"
                      class="flex items-center"
                      to="/marketplace">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5"
                   viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349m-16.5 11.65V9.35m0 0a3.001 3.001 0 003.75-.615A2.993 2.993 0 009.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 002.25 1.016c.896 0 1.7-.393 2.25-1.016a3.001 3.001 0 003.75.614m-16.5 0a3.004 3.004 0 01-.621-4.72L4.318 3.44A1.5 1.5 0 015.378 3h13.243a1.5 1.5 0 011.06.44l1.19 1.189a3 3 0 01-.621 4.72m-13.5 8.65h3.75a.75.75 0 00.75-.75V13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75v3.75c0 .415.336.75.75.75z" stroke-linecap="round"
                      stroke-linejoin="round"/>
              </svg>
              <span v-if="isDrawerOpen">{{ $t('nav.marketplace') }}</span>
            </NuxtLink>
          </li>
          <li>
            <NuxtLink :class="isDrawerOpen ? 'gap-3' : 'justify-center tooltip tooltip-right'" :data-tip="isDrawerOpen ? '' : $t('nav.messages')"
                      class="flex items-center"
                      to="/messages">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5"
                   viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" stroke-linecap="round"
                      stroke-linejoin="round"/>
              </svg>
              <span v-if="isDrawerOpen">{{ $t('nav.messages') }}</span>
            </NuxtLink>
          </li>
          <li>
            <NuxtLink :class="isDrawerOpen ? 'gap-3' : 'justify-center tooltip tooltip-right'" :data-tip="isDrawerOpen ? '' : $t('nav.settings')"
                      class="flex items-center"
                      to="/settings">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5"
                   viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z" stroke-linecap="round"
                      stroke-linejoin="round"/>
                <path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
              <span v-if="isDrawerOpen">{{ $t('nav.settings') }}</span>
            </NuxtLink>
          </li>
        </ul>
      </aside>
    </div>
  </div>
</template>

<script lang="ts" setup>
const {locale, locales, setLocale} = useI18n()
const colorMode = useColorMode()
const {user, isAuthenticated, logout} = useAuth()
const router = useRouter()

const isDrawerOpen = ref(true)

const containerClasses = computed(() => [
  'min-h-screen',
  isAuthenticated.value ? 'drawer lg:drawer-open' : '',
  {
    'is-drawer-open': isDrawerOpen.value && isAuthenticated.value,
    'is-drawer-close': !isDrawerOpen.value && isAuthenticated.value
  }
])

const availableLocales = computed(() => {
  return locales.value
})

const toggleTheme = () => {
  colorMode.preference = colorMode.value === 'dark' ? 'light' : 'dark'
}

const toggleDrawer = () => {
  isDrawerOpen.value = !isDrawerOpen.value
}

const handleLogout = async () => {
  await logout()
  router.push('/login')
}
</script>
